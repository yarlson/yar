...
- Struct field visibility and literal construction are package-owned: external selectors require `pub` fields, any private field blocks external struct literals, private fields may use private types, and generic instantiations retain the declaration's visibility. Enum payload fields remain inherently public and reject `pub`. Zero-value declarations and aggregate zero-initialization of imported private-field structs remain separate zero-value/initialization design work.
