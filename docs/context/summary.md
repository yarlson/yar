...
- Package identity is a source origin plus a source-relative subpath. Imports resolve within the importer origin: same-origin packages first, then aliases declared by that origin, then embedded stdlib; stdlib imports remain sealed to the stdlib origin. Equal logical paths from different origins therefore do not share graph identity or internal symbols.
