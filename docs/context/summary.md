...
- `internal/stdlib` embeds the standard library written in Yar (`strings`, `utf8`, `conv`, `sort`, `path`, `fs`, `process`, `env`, `stdio`, `net`, `http`, and `testing`) and provides lookup functions for the package loader.
