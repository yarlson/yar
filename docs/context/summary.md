...
- The Rust frontend and `docs/YAR.md` own language truth; `testdata/syntax_surface` is the portable accepted-syntax compatibility input provided for independently owned editor and parser projections.
...
- Programs are package graphs rooted at an entry `package main`, with one or more `.yar` files per package, explicit `import` declarations, package-qualified cross-package references, top-level `struct`, `interface`, `enum`, `error`, `fn`, and method declarations, optional `pub` on exported structs, interfaces, enums, errors, functions, and methods, and package-private-by-default struct fields with explicit `pub` field export.
...
- Declare package-owned errors with `error Name` or `pub error Name`; use local
  `error.Name` and imported public `pkg.Name` as general expressions. Unknown
  spellings are rejected and equal leaf names from different package origins
  remain distinct.
