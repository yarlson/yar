...
- `internal/codegen` lowers the checked AST into LLVM IR, expanding error sugar into explicit checks, branches, and returns, lowering loops and aggregate values, and generating the exported `main` wrapper around `yar.main`.
